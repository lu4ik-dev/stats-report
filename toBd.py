...
# Массив данных с регионами и городами
data = [
  {
...
for row in data:
    ...
    # Проверка наличия региона в таблице regions
    cursor.execute("SELECT * FROM regions WHERE text = %s", (region,))
    
    if not cursor.fetchall():
        # Региона нет в таблице, добавляем
        cursor.execute("INSERT INTO regions (text) VALUES (%s)", (region,))
        conn.commit()

    # Получаем id региона
    cursor.execute("SELECT id FROM regions WHERE text = %s", (region,))
    region_id = cursor.fetchone()[0]

    # Вставка данных в таблицу cities с привязкой к региону
    city = row['city']
    cursor.execute("INSERT INTO cities (id_region, text) VALUES (%s, %s)", (region_id, city))
    conn.commit()
